...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    public boolean addUser(String name) {
        if(getUserCount() >= users.length || getUser(name) != null){
            return false;
        }
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for (int i = 0; i < userCount; i++) {
            // 1. Skip the user themselves
            if (users[i].getName().equals(name)) {
                continue;
            }

            // 2. Skip people they already follow
    ...
    public String mostPopularUser() {
        ...
        int popScore = followeeCount(PopularUser.getName());
        
        for(int i = 1; i < getUserCount(); i++ ){
            int tmpScore = followeeCount(users[i].getName());
            if(popScore < tmpScore){
                popScore = tmpScore;
                PopularUser = users[i];
            }
        }
